...
| P4-RES-EXTERNAL-SAFE | No detach, uninstall, Profile delete, recovery, or interruption modifies external content | threat model | `packages/core/src/domain.js` | byte-for-byte Core negative tests | M1 core manifest | `docs/user/core-state.md` | implemented at Core relation layer; M2/M3 lifecycle pending |
